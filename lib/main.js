...
module.exports.start = async () => {
    ...
    if (config.localRepoDir != undefined) {
    ...
    for (let i = 0; i < workflows.length; i++) {
        ...
        if (statusRotate() || statusSummarized() || config.watch == workflow.name) {
            status.push({
...
function statusBranch() {
    return config.watch && config.watch == "branch"
}

function statusRotate() {
    return config.watch && config.watch == "rotate";
}

function statusSummarized() {
    return config.watch == undefined || config.watch && config.watch == "summarized";
}
